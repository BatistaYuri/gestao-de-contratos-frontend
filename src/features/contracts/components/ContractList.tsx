...
export function ContractList({ contracts, isLoading, errorMessage, onEdit, onDelete, onHistory, onReject, onChanged }: Props) {
  ...
  return <>{message && <p className={`form-message ${isError ? 'error' : 'success'}`} role={isError ? 'alert' : 'status'}>{message}</p>}<div className="table-wrapper"><table><thead><tr><th>Número</th><th>Cliente</th><th>Tipo</th><th>Valor</th><th>Vencimento</th><th>Vigência</th><th>Aprovação</th><th className="actions-column">Ações</th></tr></thead><tbody>
    {contracts.map((contract) => { const editable = contract.approvalStatus === 'DRAFT' || contract.approvalStatus === 'REJECTED'; const busy = Boolean(processingId); return <tr key={contract.id}><td>{contract.number}</td><td>{contract.client.name}</td><td>{contractTypeLabels[contract.type]}</td><td><span>{money.format(Number(contract.value))}</span><small className="financial-details">Subtotal {money.format(Number(contract.subtotal))}<br />Desconto {money.format(Number(contract.discount))}<br />Taxas {money.format(Number(contract.additionalFees))}</small><details className="contract-items-summary"><summary>{contract.items.length} {contract.items.length === 1 ? 'item' : 'itens'}</summary><ul>{contract.items.map((item, index) => <li key={item.id ?? index}>{item.description}: {item.quantity} × {money.format(Number(item.unitPrice))}</li>)}</ul></details></td><td>{date.format(new Date(contract.dueDate))}</td><td><ContractStatusBadge kind="term" status={contract.status} /></td><td><ContractStatusBadge kind="approval" status={contract.approvalStatus} /></td><td className="actions-column"><ActionMenu label={`Abrir ações do contrato ${contract.number}`}>
      {editable && <><button type="button" onClick={() => onEdit(contract)} disabled={busy}>Editar</button><button type="button" onClick={() => void run(contract, 'submit')} disabled={busy}>{processingId === contract.id ? 'Processando...' : 'Enviar para aprovação'}</button></>}
      {contract.approvalStatus === 'PENDING' && <><button type="button" onClick={() => void run(contract, 'approve')} disabled={busy}>{processingId === contract.id ? 'Processando...' : 'Aprovar'}</button><button type="button" className="danger-menu-item" onClick={() => onReject(contract)} disabled={busy}>Rejeitar</button></>}
      {contract.approvalStatus === 'APPROVED' && contract.status !== 'CLOSED' && <button type="button" onClick={() => void run(contract, 'close')} disabled={busy}>{processingId === contract.id ? 'Processando...' : 'Encerrar'}</button>}
